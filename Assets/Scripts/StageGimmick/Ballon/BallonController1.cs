...
public class BallonController1 : MonoBehaviour, IAbilityDetectable, IResetable
{
    [LabelText("移動距離")]
    ...
    private void PlayCurrentMotion()
    {
        if(_isUp)
        {
            ...
            _upMoveMotion = LMotion
                .Create(_startPos.y, _endPos.y, _moveDuration)
        ...
        else
        {
            ...
            _downMoveMotion = LMotion
                .Create(_endPos.y, _startPos.y, _moveDuration)
                .WithEase(Ease.InOutCubic)
                .WithOnComplete(async () =>
                {
                    CancellationTokenSource token = new CancellationTokenSource();
